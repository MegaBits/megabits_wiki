...
let cmd_dvips tmpprefix = "/usr/texbin/dvips -q -R -E " ^ tmpprefix ^ ".dvi -f >" ^ tmpprefix ^ ".ps"
let cmd_latex tmpprefix = "/usr/texbin/latex " ^ tmpprefix ^ ".tex >/dev/null"
...
let cmd_dvipng tmpprefix finalpath backcolor = "/usr/texbin/dvipng -bg \'" ^ backcolor ^ "\' -gamma 1.5 -D 120 -T tight --strict " ^ tmpprefix ^ ".dvi -o " ^ finalpath ^ " >/dev/null 2>/dev/null"
